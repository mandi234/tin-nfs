...
int mynfs_write(int fd, void *buf, int count)
{
    ...
    std::cout << "RECEIVED WRITE RESPONSE - NICE ! " << std::endl;

    memcpy(buf, res_message->buf, res_message->buf_len);
    return res_message->buf_len;
}

int mynfs_lseek(int fd, off_t offset, int whence) //return jest do poprawy. tam ma byc offset od poczatku, ale musze ogarnac jak go wyliczyc
{
    RequestOperation req_message;
    ResponseOperation *res_message;
    char response[4096];
    ...
    std::cout << "RECEIVED LSEEK RESPONSE - NICE ! " << std::endl;
